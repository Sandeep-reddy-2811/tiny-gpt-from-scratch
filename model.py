...
def naive_softmax_1d(logits):
    ...
    exp_logit = array_exp(logits)
    total = sum_all(exp_logit)
...
def encode_corpus_to_int_array(text, stoi):
    ...
    li = np.array(encode_string(text, stoi))
    return li
    pass
...
def sample_random_batch_offsets(data_len, block_size, batch_size, rng):
    ...
    if max_start < 0:
        raise ValueError("Block size too large for given data length.")
...
def allocate_count_matrix(vocab_size):
    ...
    arr = np.zeros((vocab_size, vocab_size), dtype=int)
    return arr
    pass
...
def add_one_smoothing(n_matrix):
    ...
    return n_matrix.astype(np.int64)+1
    pass
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    for t in range(1, length):
        seq[t] = sample_next_token(p_matrix, seq[t-1], rng)
    return seq
    pass
...
def decode_generated_sequence(ids, itos):
    ...
    ids = np.asarray(ids, dtype=int).tolist()

    decoded_chars = [itos[i] for i in ids]
...
def log_prob_of_pair(p_matrix, current_id, next_id):
    ...
    it = index_element(p_matrix, current_id, next_id)
    return array_log(it)
    pass
...
def average_nll(p_matrix, data):
    ...
    length = len(data) - 1
    return total / length if length > 0 else float("nan")
    pass
...
def initialize_w_random(vocab_size, rng):
    ...
    W = rng.standard_normal((vocab_size, vocab_size), dtype=float)
    return W
    pass
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    N = len(ids)
    arr = make_2d_zeros(N, vocab_size)
    arr[np.arange(N), ids] = 1.0
